...
# 中文字体设置
plt.rcParams['font.sans-serif'] = ['Arial Unicode MS', 'Heiti TC', 'STHeiti', 'Microsoft YaHei', 'PingFang SC']
...
def prepare_features(df, prediction_days=5, feature_selection=True, variance_threshold=0.001, 
    ...
    # 2. 价格类特征
    result_df['price_range'] = (result_df['high'] - result_df['low']) / result_df['low']  # 当日价格区间
    result_df['price_momentum'] = result_df['close'] / result_df['open'] - 1  # 当日价格动量
    ...
    result_df['monthly_effect'] = result_df['change'].rolling(window=20).mean()  # 月效应
    ...
    # 7. 价格与均线的相对关系
    for window in [5, 10, 20, 30, 60]:
        result_df[f'close_to_sma_{window}'] = result_df['close'] / result_df[f'sma_{window}'] - 1
        result_df[f'close_to_ema_{window}'] = result_df['close'] / result_df[f'ema_{window}'] - 1
    
    # 8. 均线交叉信号
    result_df['sma_5_10_cross'] = (result_df['sma_5'] > result_df['sma_10']).astype(int)
    result_df['sma_10_20_cross'] = (result_df['sma_10'] > result_df['sma_20']).astype(int)
    result_df['ema_5_10_cross'] = (result_df['ema_5'] > result_df['ema_10']).astype(int)
    result_df['ema_10_20_cross'] = (result_df['ema_10'] > result_df['ema_20']).astype(int)
    ...
    result_df['volatility_ratio_5_20'] = result_df['volatility_5d'] / result_df['volatility_20d']
    ...
    # 12. 趋势特征
    result_df['trend_5d'] = (result_df['close'] > result_df['close'].shift(5)).astype(int)
    result_df['trend_10d'] = (result_df['close'] > result_df['close'].shift(10)).astype(int)
    result_df['trend_20d'] = (result_df['close'] > result_df['close'].shift(20)).astype(int)
    
    # 13. 价格突破特征
    for window in [5, 10, 20, 30]:
        # 上轨突破
        upper_band = result_df['close'].rolling(window=window).max()
        result_df[f'upper_breakout_{window}'] = (result_df['close'] >= upper_band).astype(int)
        
        # 下轨突破
        lower_band = result_df['close'].rolling(window=window).min()
        result_df[f'lower_breakout_{window}'] = (result_df['close'] <= lower_band).astype(int)
    ...
    if 'upper_band' in result_df.columns and 'lower_band' in result_df.columns:
        result_df['bb_width'] = (result_df['upper_band'] - result_df['lower_band']) / result_df['middle_band']
        result_df['bb_position'] = (result_df['close'] - result_df['lower_band']) / (result_df['upper_band'] - result_df['lower_band'])
        result_df['bb_squeeze'] = ((result_df['bb_width'] < result_df['bb_width'].shift(5)) & 
                                 (result_df['bb_width'] < 0.1)).astype(int)
    ...
    if 'peTTM' in result_df.columns and 'pbMRQ' in result_df.columns:
        ...
        result_df['pe_percentile'] = result_df['peTTM'].rolling(window=60).apply(
            lambda x: pd.Series(x).rank(pct=True).iloc[-1], raw=False)
        result_df['pb_percentile'] = result_df['pbMRQ'].rolling(window=60).apply(
            lambda x: pd.Series(x).rank(pct=True).iloc[-1], raw=False)
    
    # 19. 指数和板块影响
    if 'index_close' in result_df.columns:
        # 与大盘的相对表现
        result_df['relative_to_index'] = result_df['close'].pct_change() - result_df['index_close'].pct_change()
        result_df['relative_strength_5d'] = (result_df['close'].pct_change(5) - 
                                          result_df['index_close'].pct_change(5))
        result_df['relative_strength_10d'] = (result_df['close'].pct_change(10) - 
                                           result_df['index_close'].pct_change(10))
    
    if 'sector_close' in result_df.columns:
        # 与板块的相对表现
        result_df['relative_to_sector'] = result_df['close'].pct_change() - result_df['sector_close'].pct_change()
        result_df['sector_relative_5d'] = (result_df['close'].pct_change(5) - 
                                        result_df['sector_close'].pct_change(5))
        result_df['sector_relative_10d'] = (result_df['close'].pct_change(10) - 
                                         result_df['sector_close'].pct_change(10))
    
    # 创建目标变量：未来N天是否上涨
    result_df['future_change'] = result_df['close'].shift(-prediction_days) / result_df['close'] - 1
    result_df['future_up'] = (result_df['future_change'] > 0).astype(int)
    ...
    # 移除不需要的特征列
    features_to_remove = ['date', 'code', 'future_change', 'open', 'high', 'low', 
    ...
    if feature_selection and len(feature_names) > 0:
        try:
            ...
            if feature_selection_method in ['kbest', 'combined'] and len(feature_names) > 0:
                # 使用SelectKBest选择最相关的特征
                from sklearn.feature_selection import SelectKBest, f_classif
                
                # 确定要选择的特征数量 (最多选择100个特征或60%的特征，以较小者为准)
                k = min(100, max(int(len(feature_names) * 0.6), min(30, len(feature_names))))
            ...
            if use_pca and len(feature_names) > 3:  # 至少需要3个特征才值得做PCA
                ...
                if isinstance(pca_components, float) and 0 < pca_components < 1:
                    ...
                    pca = PCA(n_components=min(int(pca_components), len(feature_names)))  # 使用指定数量的组件
                
                X_pca = pca.fit_transform(X_scaled)
                
                # 创建新的特征名
                new_feature_names = [f'PCA_Component_{i+1}' for i in range(X_pca.shape[1])]
            ...
            print(f"特征选择后，保留了 {len(feature_names)} 个特征 (从 {original_feature_count} 减少)")
        except Exception as e:
            print(f"特征选择过程出错: {e}")
            traceback.print_exc()
...
def train_ml_model(X, y, model_type='rf', feature_names=None, use_cv=True, use_feature_selection=True, use_hyperopt=False):
    """
    训练并评估机器学习模型
    ...
    try:
        ...
        if use_feature_selection and feature_names is not None:
            try:
                ...
                # 显示被保留和移除的特征数
                print(f"选中了 {len(selected_features)} 个特征，移除了 {len(feature_names) - len(selected_features)} 个低方差特征。")
        ...
        if use_cv and not use_hyperopt:  # 如果使用了超参数优化，已经做过交叉验证了
            try:
                ...
                scores = cross_val_score(model, X_train_selected, y_train_mapped, cv=tscv, scoring='accuracy')
                ...
                if hasattr(model, 'predict_proba'):
                    ...
                    if model_type in ['xgb', 'lgbm']:
                        ...
                        if y_pred_proba.shape[1] >= 3:
                            positive_class_proba = y_pred_proba[:, 2]  # 第3列对应标签1（上涨）
                        else:
...
def train_ensemble_model(X, y, ensemble_type='voting', base_models=None, feature_names=None):
    """
    训练集成模型
    ...
    try:
        ...
        # 二分类问题时计算ROC-AUC
        if len(np.unique(y_test_original)) == 2 and hasattr(ensemble_model, 'predict_proba'):
            try:
                # 对于二分类问题，获取正类的概率
                y_pred_proba = ensemble_model.predict_proba(X_test_scaled)
...
def predict_signals(features, predictions, stock_code):
    ...
    try:
        ...
        # 绘制交易信号
        ax.scatter(buy_signals, buy_prices, color='green', label='买入信号(多)', marker='^', s=100)
        ax.scatter(sell_signals, sell_prices, color='red', label='卖出信号(空)', marker='v', s=100)
        
        # 获取置信度信息
        if '预测概率' in features.columns:
            high_conf = features['预测概率'].iloc[test_start_idx:].reset_index(drop=True)
            high_conf_signals = dates[high_conf > 0.7].values
            high_conf_prices = close_prices.iloc[high_conf > 0.7].values
            ax.scatter(high_conf_signals, high_conf_prices, color='blue', label='高置信度预测', marker='*', s=120, alpha=0.5)
        
        # 添加均线作为参考
        if 'ma20' in features.columns:
            ax.plot(dates, features['ma20'].iloc[test_start_idx:].reset_index(drop=True), '--', color='purple', alpha=0.7, linewidth=1, label='20日均线')
        if 'ma10' in features.columns:
            ax.plot(dates, features['ma10'].iloc[test_start_idx:].reset_index(drop=True), '--', color='orange', alpha=0.7, linewidth=1, label='10日均线')
...
def visualize_prediction(df, stock_code, prediction_days, model_type=None):
    ...
    try:
        ...
        # 备选中文字体列表
        chinese_fonts = ['Arial Unicode MS', 'PingFang HK', 'Heiti TC', 'STHeiti', 'SimSong', 'STFangsong']
        ...
        for font_name in chinese_fonts:
            try:
                # 尝试获取这个字体的字体文件路径
                font_path = mpl.font_manager.findfont(font_name, fallback_to_default=False)
                if font_path:
                    # 创建字体属性对象
        ...
        plt.plot(df['trade_date'], df['close'], label='Close' if font_prop is None else '收盘价', linewidth=1.5)
        
        # 标记买入信号
        buy_signals = df[df['预测信号'] == 1]
        plt.scatter(buy_signals['trade_date'], buy_signals['close'], 
                   color='green', label='Buy Signal' if font_prop is None else '买入信号', marker='^', s=100)
        
        # 标记卖出信号
        sell_signals = df[df['预测信号'] == -1]
        plt.scatter(sell_signals['trade_date'], sell_signals['close'], 
                   color='red', label='Sell Signal' if font_prop is None else '卖出信号', marker='v', s=100)
        
        # 如果有置信度信息，显示置信度高的点
        if '预测概率' in df.columns:
            high_conf = df[df['预测概率'] > 0.7]
            plt.scatter(high_conf['trade_date'], high_conf['close'], 
                      color='blue', label='High Confidence' if font_prop is None else '高置信度预测', marker='*', s=120, alpha=0.5)
        
        # 添加均线作为参考
        plt.plot(df['trade_date'], df['SMA20'], '--', color='purple', alpha=0.7, linewidth=1, 
                label='20-day MA' if font_prop is None else '20日均线')
        plt.plot(df['trade_date'], df['SMA10'], '--', color='orange', alpha=0.7, linewidth=1, 
                label='10-day MA' if font_prop is None else '10日均线')
        
        # 添加标题和标签 - 使用正确的语言版本
        model_info = f"Model: {model_type}" if model_type else ""
        if font_prop:
            title = f'{stock_code} 股价预测 (预测天数: {prediction_days}天) {model_info}'
            xlabel = '日期'
            ylabel = '价格'
        else:
            title = f'{stock_code} Stock Prediction ({prediction_days}-day) {model_info}'
            xlabel = 'Date'
            ylabel = 'Price'
        ...
        plt.plot(df['trade_date'], df['signal'], label='Actual Signal' if font_prop is None else '实际信号', color='blue', linewidth=1.5)
        plt.plot(df['trade_date'], df['预测信号'], label='Predicted Signal' if font_prop is None else '预测信号', color='orange', linewidth=1.5)
        
        # 使用字体属性设置标题和标签
        if font_prop:
            plt.title('Actual vs Predicted Signals', fontproperties=font_prop, fontsize=14)
            plt.xlabel('日期', fontproperties=font_prop, fontsize=12)
            plt.ylabel('信号 (1=买入, 0=持有, -1=卖出)', fontproperties=font_prop, fontsize=12)
        else:
        ...
        # 设置y轴刻度标签
        if font_prop:
            plt.yticks([-1, 0, 1], ['卖出(-1)', '持有(0)', '买入(1)'], fontproperties=font_prop)
        else:
        ...
        # 保存高分辨率图像
        plt.savefig(f'images/{stock_code}_ml_prediction_{prediction_days}days.png', dpi=300, bbox_inches='tight')
        print(f"图表已保存至: images/{stock_code}_ml_prediction_{prediction_days}days.png")
        plt.close()
...
def evaluate_prediction_performance(df, prediction_days):
    ...
    confidence_text = f" (置信度: {last_confidence:.2f})" if '预测概率' in df.columns else ""
    ...
    if len(buy_returns) > 0:
        ...
        print(f"应用止损({stop_loss*100}%)止盈({stop_profit*100}%)后的平均收益率: {avg_adjusted_return:.2f}%")
...
def calculate_technical_indicators(df):
    ...
    try:
        ...
        result_df['bollinger_upper'] = result_df['bollinger_mid'] + 2 * result_df['bollinger_std']
        result_df['bollinger_lower'] = result_df['bollinger_mid'] - 2 * result_df['bollinger_std']
        result_df['bollinger_width'] = (result_df['bollinger_upper'] - result_df['bollinger_lower']) / result_df['bollinger_mid']
        
        # 5. 相对于均线的位置百分比
        result_df['ma5_position'] = (result_df['close'] - result_df['ma5']) / result_df['ma5'] * 100
        result_df['ma10_position'] = (result_df['close'] - result_df['ma10']) / result_df['ma10'] * 100
        result_df['ma20_position'] = (result_df['close'] - result_df['ma20']) / result_df['ma20'] * 100
        result_df['ma30_position'] = (result_df['close'] - result_df['ma30']) / result_df['ma30'] * 100
        result_df['ma60_position'] = (result_df['close'] - result_df['ma60']) / result_df['ma60'] * 100
        ...
        result_df['momentum_10d'] = result_df['close'] / result_df['close'].shift(10) - 1
        result_df['momentum_20d'] = result_df['close'] / result_df['close'].shift(20) - 1
        ...
        for i in range(9, len(result_df)):
            if i < len(result_df) and i - 1 >= 0 and i < len(high_max) and i < len(low_min):
                if high_max.iloc[i] - low_min.iloc[i] > 0:
                    if i < len(result_df.index) and i-1 < len(result_df.index):
                        result_df.loc[result_df.index[i], 'kdj_k'] = result_df.loc[result_df.index[i-1], 'kdj_k'] * 2/3 + 100 * (result_df.loc[result_df.index[i], 'close'] - low_min.iloc[i]) / (high_max.iloc[i] - low_min.iloc[i]) * 1/3
                        result_df.loc[result_df.index[i], 'kdj_d'] = result_df.loc[result_df.index[i-1], 'kdj_d'] * 2/3 + result_df.loc[result_df.index[i], 'kdj_k'] * 1/3
        
        result_df['kdj_j'] = 3 * result_df['kdj_k'] - 2 * result_df['kdj_d']
        
        # 12. Williams %R指标
        result_df['williams_r'] = (high_max - result_df['close']) / (high_max - low_min) * -100
        ...
        # 14. Ichimoku云图指标
        result_df['ichimoku_tenkan'] = (result_df['high'].rolling(window=9).max() + result_df['low'].rolling(window=9).min()) / 2
        result_df['ichimoku_kijun'] = (result_df['high'].rolling(window=26).max() + result_df['low'].rolling(window=26).min()) / 2
        
        # 处理财务指标
        if 'peTTM' in result_df.columns:
            result_df['pe_percentile'] = result_df['peTTM'].rolling(60).apply(lambda x: pd.Series(x).rank(pct=True).iloc[-1], raw=True)
        if 'pbMRQ' in result_df.columns:
            result_df['pb_percentile'] = result_df['pbMRQ'].rolling(60).apply(lambda x: pd.Series(x).rank(pct=True).iloc[-1], raw=True)
...
def main(args=None):
    ...
    parser.add_argument('-s', '--stock', type=str, required=True, help='股票代码，如 sh.600036')
    parser.add_argument('-d', '--days', type=int, default=365, help='历史数据天数，默认365天')
    parser.add_argument('-f', '--forecast', type=int, default=5, help='预测未来几天的走势，默认5天')
    parser.add_argument('-m', '--model', type=str, default='rf', choices=['rf', 'xgb', 'lgbm', 'voting', 'stacking'], 
    ...
    parser.add_argument('--feature-method', type=str, default='variance', choices=['variance', 'kbest', 'combined'], 
                        help='特征选择方法: variance=方差选择, kbest=K优特征, combined=两者结合')
    parser.add_argument('--variance-threshold', type=float, default=0.001, help='方差特征选择阈值，默认0.001')
    parser.add_argument('--correlation-threshold', type=float, default=0.85, help='相关性特征选择阈值，默认0.85，数值越小越严格')
    parser.add_argument('--use-pca', action='store_true', help='使用PCA降维')
    parser.add_argument('--pca-components', type=float, default=0.95, help='PCA保留方差比例(0-1)或组件数量(>1)')
    parser.add_argument('--hyperopt', action='store_true', help='使用超参数优化')
    parser.add_argument('--learning-rate', type=float, default=0.1, help='学习率，用于XGBoost和LightGBM')
    parser.add_argument('--num-leaves', type=int, default=31, help='叶子节点数，用于LightGBM')
    ...
    if args.model in ['voting', 'stacking']:
        ...
        model, predictions, accuracy, cm, feature_importance = train_ensemble_model(
            X, y, ensemble_type=args.model, base_models=None, feature_names=feature_names
        )
    else:
        # 单一模型
        model, predictions, accuracy, cm, feature_importance = train_ml_model(
            X, y, model_type=args.model, feature_names=feature_names, 
            use_cv=not args.no_cv, use_feature_selection=args.feature_selection,
